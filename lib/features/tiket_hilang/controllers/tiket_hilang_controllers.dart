...
class LostTicketController {
  ...
  Future<void> printLostTicketReceipt(BuildContext context) async {
    try {
      // Ambil data kasir dan nama lokasi dari storage
      final username = await _storageService.read('username') ?? 'kasir';
      final locationName = await _storageService.read('locationName') ?? 'Lokasi Parkir';

      // Kirim semua data yang diperlukan ke printer service
      await _printerService.printLostTicketReceipt(
        data: ticket,
        kasir: username,
